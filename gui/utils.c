...
time_t parse_deadline_time(GtkSpinButton *time_entry, GtkCalendar *cal_entry){
  time_t now = time(0);
...
void getTimeRemainedStr(time_t deadline, char *buf, size_t n){
  ...
  int64_t t_diff = (int64_t)difftime(deadline, now);
  ...
  t_diff %= 60;
  seconds = t_diff;
  int r = snprintf(buf, n, "%d:%d:%d", hours, minutes, seconds);
}